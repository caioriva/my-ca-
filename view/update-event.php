...
<html>
    <head>
        ...
        <script type='text/javascript'>
            ...
            function validateCreateEventsForm() {
                ...
                var eventTypeValue = document.getElementById("event-type")
                        .options[document.getElementById("event-type").selectedIndex].value;
    ...
    <body>
        ...
        <div class="wrapper" name="login-form" style="margin-top: -145px">
            <div class="inner-wrapper"> 
                <form id="create-event-form" action="controller/update-event-controller.php" method="post" onsubmit="javascript:return validateCreateEventsForm()">
                    <div class="message-wrapper" style="margin-bottom: 20px">
                        <h2 style="text-align: center; color: rgb(102, 102, 102)">Update Your Event</h2>
                    </div>
                    <?php
                    if (session_status() == PHP_SESSION_NONE) {
                        session_start();
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "GET") {
                        ...
                        if (isset($_GET['event'])) {
                            ...
                            echo("<input type='hidden' name='event-id' value='$eventId'/>");
                            
                            echo("<input class='general-form-input' type='text' name='event-title' 
                                placeholder='Event Title' autocomplete='off' title='Event Title' 
                                maxlength='50' value='$eventTitle'  required/>");
                            ...
                            if($event->getEventType() == $allDayName) {
                                ...
                                echo("<select id='event-type' class='general-form-input' name='event-type' autocomplete='off' 
                                        onchange='javascript:showInputByEventType(this.options[this.selectedIndex].value);' required>
                                            ...
                                            <option value='$allDayName' selected>$allDayValue</option>
                                    </select>
                                    ...
                                    <div id='$allDayName'>
                                        <input id='event-date' class='general-form-input' type='date' name='event-date' 
                                            placeholder='Event Date' autocomplete='off' title='Event' value='$eventDate'/>
                                    </div>");
                            } else {
                                ...
                                echo("<select id='event-type' class='general-form-input' name='event-type' autocomplete='off' 
                                        onchange='javascript:showInputByEventType(this.options[this.selectedIndex].value);' required>
                                            <option value='$fixedTimeName' selected>$fixedTimeValue</option>
                                            <option value='$allDayName'>$allDayValue</option>
                                    </select>
                                    <div id='$fixedTimeName'>
                                        <input id='event-start-time' class='general-form-input' type='datetime-local' name='event-start-time' 
                                            placeholder='Event Start Time' autocomplete='off' title='Event Start Date/Time' value='$eventStartTime'/>
                                        <input id='event-end-time' class='general-form-input' type='datetime-local' name='event-end-time' 
                                            placeholder='Event End Time' autocomplete='off' title='Event End Date/Time' value='$eventEndTime'/>
                                    </div>
                                    <div id='$allDayName'>
                                        <input id='event-date' class='general-form-input' type='date' name='event-date' 
                    ...
                    <input class="general-form-input" type="submit" value="Update" onclick="sendEventId()">
                </form>
            </div>
        </div>
    </body>
